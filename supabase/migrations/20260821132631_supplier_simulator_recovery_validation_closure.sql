...
RETURNS trigger LANGUAGE plpgsql SET search_path TO '' AS $$
BEGIN
  IF OLD.status IN ('passed','failed') THEN RAISE EXCEPTION 'terminal supplier simulator run is immutable'; END IF;
  IF OLD.environment IS DISTINCT FROM NEW.environment OR OLD.run_key IS DISTINCT FROM NEW.run_key OR OLD.simulator_version IS DISTINCT FROM NEW.simulator_version OR OLD.contract_version IS DISTINCT FROM NEW.contract_version OR OLD.required_checks IS DISTINCT FROM NEW.required_checks OR OLD.started_by IS DISTINCT FROM NEW.started_by OR OLD.started_at IS DISTINCT FROM NEW.started_at THEN
...
BEGIN
  ...
  IF v_status='passed' THEN
    ...
    IF NOT EXISTS(SELECT 1 FROM private.supplier_replay_validation_evidence e WHERE e.run_id=p_run_id AND e.replay_class='supplier_submit' AND e.result='recovered') THEN RETURN jsonb_build_object('ok',false,'reason','lost_response_recovery_not_proven','interfaceVersion',1); END IF;
    IF NOT EXISTS(SELECT 1 FROM private.supplier_replay_validation_evidence e WHERE e.run_id=p_run_id AND e.result='blocked_collision') THEN RETURN jsonb_build_object('ok',false,'reason','idempotency_collision_block_not_proven','interfaceVersion',1); END IF;
    IF NOT EXISTS(SELECT 1 FROM private.supplier_replay_validation_evidence e WHERE e.run_id=p_run_id AND e.replay_class IN ('acknowledgement','tracking','refund','supplier_recovery') AND e.result='exact_replay' GROUP BY e.run_id HAVING count(DISTINCT e.replay_class)=4) THEN RETURN jsonb_build_object('ok',false,'reason','canonical_replay_classes_not_proven','interfaceVersion',1); END IF;
  END IF;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'runs',(SELECT COALESCE(jsonb_agg(to_jsonb(r) ORDER BY r.started_at DESC),'[]'::jsonb) FROM (SELECT r.* FROM private.supplier_simulator_validation_runs r WHERE p_run_id IS NULL OR r.id=p_run_id ORDER BY r.started_at DESC LIMIT 25) r),
