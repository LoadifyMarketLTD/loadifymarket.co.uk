...
BEGIN
  ...
  END IF;
  SELECT * INTO v_global FROM private.supplier_commerce_controls WHERE operation='*' AND scope_type='global' AND scope_ref IS NULL LIMIT 1;
  SELECT * INTO v_operation_control FROM private.supplier_commerce_controls WHERE operation=v_operation AND scope_type='global' AND scope_ref IS NULL LIMIT 1;
  IF v_global.id IS NOT NULL AND v_global.enabled=true AND v_operation_control.id IS NOT NULL AND v_operation_control.enabled=true THEN
    FOREACH v_scope_type IN ARRAY ARRAY['provider','supplier','offer','product','category','territory','cohort']::text[] LOOP
      v_scope_key:=CASE v_scope_type WHEN 'provider' THEN 'providerRef' WHEN 'supplier' THEN 'supplierRef' WHEN 'offer' THEN 'offerRef'
        WHEN 'product' THEN 'productRef' WHEN 'category' THEN 'categoryRef' WHEN 'territory' THEN 'territory' WHEN 'cohort' THEN 'cohort' END;
...
CREATE TABLE IF NOT EXISTS private.supplier_order_cancellations (
  ...
  CONSTRAINT supplier_order_cancellation_state_check CHECK (state IN ('prepared','submitting','retryable_failure','rejected','unknown','reconciliation_required','cancelled','manual_review')),
...
CREATE INDEX IF NOT EXISTS supplier_order_cancellation_order_idx ON private.supplier_order_cancellations(order_id,state,updated_at DESC);
...
CREATE INDEX IF NOT EXISTS supplier_order_cancellation_event_idx ON private.supplier_order_cancellation_events(cancellation_id,created_at);
...
BEGIN
  IF TG_OP='UPDATE' AND (
    ...
    OR NEW.external_supplier_order_ref IS DISTINCT FROM OLD.external_supplier_order_ref OR NEW.cancellation_key IS DISTINCT FROM OLD.cancellation_key
    OR NEW.correlation_id IS DISTINCT FROM OLD.correlation_id OR NEW.request_fingerprint IS DISTINCT FROM OLD.request_fingerprint OR NEW.created_at IS DISTINCT FROM OLD.created_at
...
CREATE TRIGGER trg_guard_supplier_order_cancellation_identity_v1 BEFORE UPDATE ON private.supplier_order_cancellations
...
RETURNS trigger LANGUAGE plpgsql SET search_path TO '' AS $$ BEGIN RAISE EXCEPTION 'supplier cancellation events are append-only'; END; $$;
...
CREATE TRIGGER trg_guard_supplier_order_cancellation_event_immutable_v1 BEFORE UPDATE OR DELETE ON private.supplier_order_cancellation_events
FOR EACH ROW EXECUTE FUNCTION private.guard_supplier_order_cancellation_event_immutable_v1();

CREATE OR REPLACE FUNCTION public.server_prepare_supplier_order_cancellation_v1(p_handshake_id uuid,p_cancellation_key text,p_correlation_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE
  v_h private.supplier_order_handshakes%ROWTYPE; v_order public.orders%ROWTYPE; v_leg private.supplier_fulfilment_legs%ROWTYPE;
  v_offer private.supplier_offers%ROWTYPE; v_supplier private.supplier_foundation_suppliers%ROWTYPE; v_adapter private.supplier_adapter_registrations%ROWTYPE;
  v_existing private.supplier_order_cancellations%ROWTYPE; v_saved private.supplier_order_cancellations%ROWTYPE; v_control jsonb;
  v_key text:=BTRIM(COALESCE(p_cancellation_key,'')); v_fingerprint text;
BEGIN
  IF p_handshake_id IS NULL OR v_key='' OR p_correlation_id IS NULL THEN RETURN jsonb_build_object('eligible',false,'reason','cancellation_identity_required','interfaceVersion',1); END IF;
  ...
  IF v_h.state NOT IN ('accepted','reconciled') OR v_h.acknowledgement_state<>'accepted' OR NULLIF(BTRIM(v_h.external_supplier_order_ref),'') IS NULL THEN
  ...
  IF NOT FOUND OR v_leg.status<>'supplier_accepted' THEN RETURN jsonb_build_object('eligible',false,'reason','supplier_leg_not_cancellable','legStatus',v_leg.status,'interfaceVersion',1); END IF;
  ...
  v_control:=public.server_supplier_commerce_control_decision_v1('cancellation',jsonb_build_object('providerRef',v_h.provider_key,'supplierRef',v_supplier.supplier_key,
    'offerRef',v_offer.id::text,'productRef',v_offer.canonical_product_id::text,'territory',v_offer.territory));
  IF COALESCE((v_control->>'enabled')::boolean,false) IS DISTINCT FROM true THEN RETURN jsonb_build_object('eligible',false,'reason','supplier_cancellation_control_disabled','control',v_control,'interfaceVersion',1); END IF;
  SELECT * INTO v_adapter FROM private.supplier_adapter_registrations a WHERE a.supplier_id=v_supplier.id AND a.provider_key=v_h.provider_key AND a.adapter_version=v_h.adapter_version
    AND a.status='active' AND a.interface_version=1 AND a.verified_at IS NOT NULL AND a.capabilities @> ARRAY['cancellation']::text[] LIMIT 1;
  ...
  v_fingerprint:=md5(concat_ws('|',v_h.id::text,v_h.order_id::text,v_h.fulfilment_leg_id::text,v_h.supplier_offer_id::text,v_h.provider_key,v_h.adapter_version,v_h.external_supplier_order_ref));
  SELECT * INTO v_existing FROM private.supplier_order_cancellations WHERE handshake_id=v_h.id FOR UPDATE;
  IF FOUND THEN
    IF v_existing.cancellation_key<>v_key OR v_existing.correlation_id<>p_correlation_id OR v_existing.request_fingerprint<>v_fingerprint THEN RAISE EXCEPTION 'supplier cancellation idempotency collision'; END IF;
    RETURN jsonb_build_object('eligible',v_existing.state NOT IN ('rejected','manual_review'),'reason','supplier_cancellation_replayed','cancellationId',v_existing.id,
      'state',v_existing.state,'providerKey',v_existing.provider_key,'adapterVersion',v_existing.adapter_version,'supplierKey',v_supplier.supplier_key,
      'territory',v_offer.territory,'externalSupplierOrderRef',v_existing.external_supplier_order_ref,'cancellationKey',v_existing.cancellation_key,
      'correlationId',v_existing.correlation_id,'interfaceVersion',1);
  END IF;
  INSERT INTO private.supplier_order_cancellations(handshake_id,order_id,orchestration_id,fulfilment_leg_id,supplier_offer_id,supplier_id,provider_key,adapter_version,
    external_supplier_order_ref,cancellation_key,correlation_id,request_fingerprint)
  VALUES(v_h.id,v_h.order_id,v_h.orchestration_id,v_h.fulfilment_leg_id,v_h.supplier_offer_id,v_h.supplier_id,v_h.provider_key,v_h.adapter_version,
    v_h.external_supplier_order_ref,v_key,p_correlation_id,v_fingerprint) RETURNING * INTO v_saved;
  ...
  RETURN jsonb_build_object('eligible',true,'reason','supplier_cancellation_ready','cancellationId',v_saved.id,'state',v_saved.state,'providerKey',v_saved.provider_key,
    'adapterVersion',v_saved.adapter_version,'supplierKey',v_supplier.supplier_key,'territory',v_offer.territory,'externalSupplierOrderRef',v_saved.external_supplier_order_ref,
    'cancellationKey',v_saved.cancellation_key,'correlationId',v_saved.correlation_id,'interfaceVersion',1);
END;
$$;
...
CREATE OR REPLACE FUNCTION public.server_mark_supplier_order_cancellation_started_v1(p_cancellation_id uuid,p_cancellation_key text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
...
BEGIN
  ...
  IF v_c.state IN ('unknown','reconciliation_required') THEN RETURN jsonb_build_object('ok',false,'reason','query_before_retry_required','state',v_c.state,'interfaceVersion',1); END IF;
  IF v_c.state IN ('rejected','manual_review') THEN RETURN jsonb_build_object('ok',false,'reason','supplier_cancellation_requires_manual_review','state',v_c.state,'interfaceVersion',1); END IF;
  IF v_c.state='submitting' THEN RETURN jsonb_build_object('ok',false,'reason','supplier_cancellation_already_in_flight','state',v_c.state,'interfaceVersion',1); END IF;
  v_previous:=v_c.state;
  UPDATE private.supplier_order_cancellations SET state='submitting',attempts=attempts+1,started_at=now(),updated_at=now() WHERE id=v_c.id RETURNING * INTO v_c;
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE v_c private.supplier_order_cancellations%ROWTYPE; v_result text:=upper(BTRIM(COALESCE(p_result_class,''))); v_previous text; v_new_state text;
BEGIN
  SELECT * INTO v_c FROM private.supplier_order_cancellations WHERE id=p_cancellation_id FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('ok',false,'reason','supplier_cancellation_not_found','interfaceVersion',1); END IF;
  ...
  IF v_result NOT IN ('SUCCESS','RETRYABLE_FAILURE','RATE_LIMITED','PERMANENT_REJECTION','AUTH_CONFIGURATION_FAILURE','UNKNOWN_OUTCOME','MANUAL_REVIEW_REQUIRED') THEN RAISE EXCEPTION 'invalid supplier cancellation result class'; END IF;
  v_previous:=v_c.state;
  v_new_state:=CASE WHEN v_result='SUCCESS' AND p_cancelled=true THEN 'cancelled' WHEN v_result IN ('RETRYABLE_FAILURE','RATE_LIMITED') THEN 'retryable_failure'
    WHEN v_result='PERMANENT_REJECTION' THEN 'rejected' WHEN v_result='UNKNOWN_OUTCOME' THEN 'reconciliation_required' ELSE 'manual_review' END;
  UPDATE private.supplier_order_cancellations SET state=v_new_state,last_error_class=NULLIF(BTRIM(p_error_class),''),last_error_message=NULLIF(BTRIM(p_error_message),''),
    completed_at=CASE WHEN v_new_state IN ('cancelled','rejected','manual_review') THEN now() ELSE NULL END,updated_at=now() WHERE id=v_c.id RETURNING * INTO v_c;
  ...
  VALUES(v_c.id,'result:'||v_c.id::text||':'||v_c.attempts::text,'result',v_previous,v_new_state,v_result,NULLIF(BTRIM(p_error_class),''),
    CASE WHEN v_new_state='cancelled' THEN 'supplier_order_cancellation_confirmed' WHEN v_new_state='reconciliation_required' THEN 'cancellation_outcome_unknown_no_blind_retry' ELSE 'supplier_cancellation_not_confirmed' END);
  RETURN jsonb_build_object('ok',v_new_state='cancelled','reason',CASE WHEN v_new_state='cancelled' THEN 'supplier_order_cancelled' ELSE 'supplier_order_cancellation_not_confirmed' END,
    'cancellationId',v_c.id,'state',v_new_state,'recoveryState',CASE WHEN v_new_state='reconciliation_required' THEN 'query_before_retry' WHEN v_new_state='retryable_failure' THEN 'retry_pending'
      WHEN v_new_state='cancelled' THEN 'resolved' ELSE 'manual_review' END,'interfaceVersion',1);
END;
$$;
...
COMMENT ON TABLE private.supplier_order_cancellations IS 'Stage 5C provider-neutral cancellation state for one already-accepted supplier order. Unknown outcomes never permit blind retry.';
COMMENT ON FUNCTION public.server_prepare_supplier_order_cancellation_v1(uuid,text,uuid) IS 'Stage 5C prepares cancellation only for an accepted supplier order, exact adapter version/capability and enabled cancellation control.';
