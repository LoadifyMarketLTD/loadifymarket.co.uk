-- ECN-3C shadow Route Engine composition with unified seller inventory-source decision.
...
DECLARE
  ...
  v_inventory_selected jsonb:='{}'::jsonb;
  v_effective_dispatch_location_id uuid;
  v_inventory_source_type text;
...
BEGIN
  ...
  IF p_supplier_id IS NULL THEN
    ...
    v_inventory:=public.server_inventory_source_decision_v1(
      p_product_id,
      v_destination_country,
      v_destination_market,
      v_quantity,
      v_context,
      p_dispatch_location_id,
      NULL,
      NULL,
      ''
    );

    v_inventory_selected:=COALESCE(
      v_inventory->'selected',
      v_inventory->'candidates'->0,
      '{}'::jsonb
    );
    v_inventory_source_type:=NULLIF(v_inventory_selected->>'sourceType','');
    v_origin:=NULLIF(v_inventory_selected->>'sourceCountry','');

    BEGIN
      v_effective_dispatch_location_id:=
        NULLIF(v_inventory_selected->>'dispatchLocationId','')::uuid;
    EXCEPTION WHEN invalid_text_representation THEN
      v_effective_dispatch_location_id:=NULL;
    END;

    v_legacy_gb_origin:=COALESCE(
      (v_inventory_selected->>'legacyGbOrigin')::boolean,
      false
    );

    BEGIN
      v_stock_ok:=
        COALESCE(NULLIF(v_inventory_selected->>'availableQuantity','')::integer,0) >= v_quantity;
    EXCEPTION WHEN invalid_text_representation OR numeric_value_out_of_range THEN
      v_stock_ok:=false;
    END;

    IF v_effective_dispatch_location_id IS NOT NULL THEN
      SELECT * INTO v_location
      FROM private.dispatch_locations
      WHERE id=v_effective_dispatch_location_id
      ...
      ELSE
        v_location_verified:=false;
        v_diagnostics:=array_append(v_diagnostics,'DISPATCH_LOCATION_MISSING');
      END IF;
    ELSIF v_legacy_gb_origin
       AND v_destination_country='GB'
    ...
    IF v_origin IS NULL THEN
      v_diagnostics:=array_append(v_diagnostics,'DISPATCH_LOCATION_MISSING');
    END IF;
    IF NOT v_location_verified AND v_origin IS NOT NULL THEN
      v_diagnostics:=array_append(v_diagnostics,'DISPATCH_LOCATION_UNVERIFIED');
    END IF;
  ELSE
    -- Supplier inventory remains fail-closed until the independent-supplier
    -- commercial/payment contract is verified. Do not compose supplier stock
    -- into ECN-3C merely from technical warehouse evidence.
    ...
    IF p_dispatch_location_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        v_origin:=v_location.country_code;
        v_location_verified:=v_location.verification_status='verified';
        v_effective_dispatch_location_id:=v_location.id;
      END IF;
    END IF;
  ...
  ELSIF v_route.id IS NOT NULL AND v_location_verified THEN
    IF p_supplier_id IS NULL THEN
      ...
      WHERE route_id=v_route.id
        ...
        AND dispatch_location_id=v_effective_dispatch_location_id
  ...
  RETURN jsonb_build_object(
    ...
    'dispatchLocationId',v_effective_dispatch_location_id,
    'inventorySource',v_inventory,
...
COMMENT ON FUNCTION public.server_cross_border_product_decision_v1(
  ...
  'ECN-3C shadow cross-border product decision composed with server_inventory_source_decision_v1 for seller-owned inventory. Supplier routing remains fail-closed until the independent-supplier commercial/payment contract is verified. Read-only and service-role-only.';
