...
END;
$$;
DROP TRIGGER IF EXISTS trg_guard_supplier_sla_supersession_v1 ON private.supplier_sla_versions;
CREATE TRIGGER trg_guard_supplier_sla_supersession_v1
BEFORE UPDATE ON private.supplier_sla_versions
FOR EACH ROW EXECUTE FUNCTION private.guard_supplier_sla_supersession_v1();
...
END;
$$;
DROP TRIGGER IF EXISTS trg_guard_supplier_adapter_evidence_v1 ON private.supplier_adapter_registrations;
CREATE TRIGGER trg_guard_supplier_adapter_evidence_v1
BEFORE INSERT OR UPDATE ON private.supplier_adapter_registrations
FOR EACH ROW EXECUTE FUNCTION private.guard_supplier_adapter_evidence_v1();
...
COMMENT ON FUNCTION private.guard_supplier_adapter_evidence_v1() IS 'Phase D fail-closed adapter activation guard. Every active adapter capability requires current, official-source-backed Phase C provider evidence for the same provider and territory.';;
