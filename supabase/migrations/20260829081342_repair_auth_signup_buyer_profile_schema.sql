...
BEGIN
  ...
  IF v_email = '' THEN RAISE EXCEPTION 'signup rejected: auth email is missing'; END IF;
  IF NEW.raw_user_meta_data ? 'role' THEN RAISE EXCEPTION 'signup rejected: client role metadata is forbidden'; END IF;

  IF v_provider = 'google' THEN
    v_provider_subject := btrim(COALESCE(NEW.raw_user_meta_data ->> 'sub', ''));
    IF v_provider_subject = '' THEN RAISE EXCEPTION 'signup rejected: verified Google subject is missing'; END IF;
    SELECT * INTO v_intent FROM private.signup_intents
    WHERE auth_provider='google' AND provider_subject=v_provider_subject AND email=v_email
      AND consumed_at IS NULL AND expires_at > now()
    ORDER BY created_at DESC LIMIT 1 FOR UPDATE;
    IF NOT FOUND THEN RAISE EXCEPTION 'signup rejected: Google registration authorization not found'; END IF;
  ...
  ELSIF v_provider = 'email' THEN
    ...
    IF v_intent_id_text <> '' THEN
      BEGIN v_intent_id := v_intent_id_text::uuid;
      EXCEPTION WHEN invalid_text_representation THEN RAISE EXCEPTION 'signup rejected: invalid signup intent'; END;
      SELECT * INTO v_intent FROM private.signup_intents
      WHERE id=v_intent_id AND auth_provider='email' AND provider_subject IS NULL FOR UPDATE;
      IF NOT FOUND THEN RAISE EXCEPTION 'signup rejected: signup intent not found'; END IF;
    ELSIF v_allow_legacy_server_registration
          AND NEW.raw_app_meta_data ? 'role'
          AND lower(btrim(COALESCE(NEW.raw_app_meta_data ->> 'role',''))) IN ('buyer','seller') THEN
      ...
      IF v_first_name='' OR v_last_name='' THEN RAISE EXCEPTION 'signup rejected: legacy server registration identity is incomplete'; END IF;
    ELSE
      IF NEW.raw_app_meta_data ? 'role' THEN RAISE EXCEPTION 'signup rejected: public email signup cannot carry app role metadata'; END IF;
  ...
  IF v_intent_id IS NOT NULL THEN
    IF v_intent.consumed_at IS NOT NULL THEN RAISE EXCEPTION 'signup rejected: signup intent already consumed'; END IF;
    IF v_intent.expires_at <= now() THEN RAISE EXCEPTION 'signup rejected: signup intent expired'; END IF;
    IF v_intent.email IS DISTINCT FROM v_email THEN RAISE EXCEPTION 'signup rejected: signup intent email mismatch'; END IF;
    IF v_intent.requested_role NOT IN ('buyer','seller') THEN RAISE EXCEPTION 'signup rejected: unsupported signup relationship'; END IF;
  ...
  IF v_effective_role NOT IN ('buyer','seller') THEN RAISE EXCEPTION 'signup rejected: signup relationship could not be derived'; END IF;

  SELECT ps.value INTO v_feature_flags FROM public.platform_settings AS ps WHERE ps.key='feature_flags';
  IF NOT FOUND
  ...
  IF v_effective_role='buyer' AND NOT v_buyer_registration THEN RAISE EXCEPTION 'signup rejected: buyer registration is temporarily disabled'; END IF;
  IF v_effective_role='seller' AND NOT v_seller_registration THEN RAISE EXCEPTION 'signup rejected: seller registration is temporarily disabled'; END IF;
  ...
  IF v_intent_id IS NOT NULL AND v_intent.requested_role='buyer' THEN
    ...
    SET "accountType"=COALESCE(NULLIF(v_intent.customer_type,''),"accountType"),
        "companyName"=COALESCE(NULLIF(v_intent.company_name,''),"companyName"),
        "vatNumber"=COALESCE(NULLIF(v_intent.vat_number,''),"vatNumber"),
