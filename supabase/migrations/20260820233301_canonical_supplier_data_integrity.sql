...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'terminal catalog dedup resolution is immutable'; END IF;
  IF NEW.decision IN ('same_product','different_product') AND NEW.evidence = '{}'::jsonb THEN
...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'different-product dedup resolution blocks supplier offer approval'; END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_guard_supplier_offer_dedup_conflict_v1 ON private.supplier_offers;
CREATE TRIGGER trg_guard_supplier_offer_dedup_conflict_v1 BEFORE INSERT OR UPDATE ON private.supplier_offers FOR EACH ROW EXECUTE FUNCTION private.guard_supplier_offer_dedup_conflict_v1();

CREATE OR REPLACE FUNCTION private.guard_supplier_catalog_identifier_namespace_v1()
RETURNS trigger LANGUAGE plpgsql SET search_path TO '' AS $$
BEGIN
  IF NULLIF(BTRIM(NEW.identifier_namespace),'') IS NULL THEN RAISE EXCEPTION 'supplier catalog identifier namespace is required'; END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_guard_supplier_catalog_identifier_namespace_v1 ON private.supplier_catalog_identifiers;
CREATE TRIGGER trg_guard_supplier_catalog_identifier_namespace_v1 BEFORE INSERT OR UPDATE ON private.supplier_catalog_identifiers FOR EACH ROW EXECUTE FUNCTION private.guard_supplier_catalog_identifier_namespace_v1();;
