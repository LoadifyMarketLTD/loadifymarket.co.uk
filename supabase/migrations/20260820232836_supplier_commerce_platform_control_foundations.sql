...
BEGIN
  ...
  END IF;
  IF v_operation NOT IN ('*','import','publish','checkout','reservation','supplier_order','tracking_ingest','return_recovery') THEN RAISE EXCEPTION 'unsupported Supplier Commerce control operation' USING ERRCODE='22023'; END IF;
  IF v_scope_type NOT IN ('global','provider','supplier','offer','product','category','territory','cohort') THEN RAISE EXCEPTION 'unsupported Supplier Commerce control scope' USING ERRCODE='22023'; END IF;
  IF (v_scope_type='global' AND v_scope_ref IS NOT NULL) OR (v_scope_type<>'global' AND v_scope_ref IS NULL) THEN RAISE EXCEPTION 'invalid Supplier Commerce control scope reference' USING ERRCODE='22023'; END IF;
  ...
  IF FOUND THEN
    UPDATE private.supplier_commerce_controls SET enabled=p_enabled,reason=v_reason,version=version+1,updated_by=p_actor_id,updated_at=now()
  ...
  END IF;
  INSERT INTO private.supplier_commerce_control_audit(control_id,actor_id,operation,scope_type,scope_ref,previous_enabled,new_enabled,previous_version,new_version,reason)
  VALUES(v_saved.id,p_actor_id,v_operation,v_scope_type,v_scope_ref,CASE WHEN v_existing.id IS NULL THEN NULL ELSE v_existing.enabled END,v_saved.enabled,CASE WHEN v_existing.id IS NULL THEN NULL ELSE v_existing.version END,v_saved.version,v_reason);
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id',c.id,'operation',c.operation,'scopeType',c.scope_type,'scopeRef',c.scope_ref,'enabled',c.enabled,'reason',c.reason,'version',c.version,'updatedBy',c.updated_by,'updatedAt',c.updated_at) ORDER BY c.operation,c.scope_type,c.scope_ref_key),'[]'::jsonb)
  INTO v_result FROM private.supplier_commerce_controls c;
  RETURN jsonb_build_object('interfaceVersion',1,'controls',v_result);
END;
$$;
REVOKE ALL ON FUNCTION public.server_list_supplier_commerce_controls_v1(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.server_list_supplier_commerce_controls_v1(uuid) TO service_role;

CREATE OR REPLACE FUNCTION public.server_record_supplier_commerce_operation_v1(
  p_correlation_id uuid,p_request_id text,p_operation text,p_provider_ref text,p_supplier_ref text,p_entity_type text,p_entity_ref text,p_result_class text,p_error_class text,p_recovery_state text,p_retry_count integer,p_external_ref text,p_customer_impact text,p_financial_impact text,p_started_at timestamptz,p_finished_at timestamptz
)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path TO '' AS $$
DECLARE v_id uuid;
BEGIN
  IF p_correlation_id IS NULL THEN RAISE EXCEPTION 'correlation id required' USING ERRCODE='22023'; END IF;
  IF NULLIF(BTRIM(COALESCE(p_operation,'')),'') IS NULL THEN RAISE EXCEPTION 'operation required' USING ERRCODE='22023'; END IF;
  INSERT INTO private.supplier_commerce_operations(correlation_id,request_id,operation,interface_version,provider_ref,supplier_ref,entity_type,entity_ref,result_class,error_class,recovery_state,retry_count,external_ref,customer_impact,financial_impact,started_at,finished_at)
  VALUES(p_correlation_id,NULLIF(BTRIM(p_request_id),''),BTRIM(p_operation),1,NULLIF(BTRIM(p_provider_ref),''),NULLIF(BTRIM(p_supplier_ref),''),NULLIF(BTRIM(p_entity_type),''),NULLIF(BTRIM(p_entity_ref),''),p_result_class,NULLIF(BTRIM(p_error_class),''),COALESCE(NULLIF(BTRIM(p_recovery_state),''),'none'),COALESCE(p_retry_count,0),NULLIF(BTRIM(p_external_ref),''),NULLIF(BTRIM(p_customer_impact),''),NULLIF(BTRIM(p_financial_impact),''),COALESCE(p_started_at,now()),p_finished_at)
  RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;
REVOKE ALL ON FUNCTION public.server_record_supplier_commerce_operation_v1(uuid,text,text,text,text,text,text,text,text,text,integer,text,text,text,timestamptz,timestamptz) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.server_record_supplier_commerce_operation_v1(uuid,text,text,text,text,text,text,text,text,text,integer,text,text,text,timestamptz,timestamptz) TO service_role;

COMMENT ON FUNCTION public.server_supplier_commerce_control_decision_v1(text,jsonb) IS 'Phase C v1 server-only Supplier Commerce rollout/kill-switch decision. Missing/invalid control evidence fails closed.';
COMMENT ON TABLE private.supplier_commerce_provider_capabilities IS 'Versioned provider/legal capability evidence register framework. Verified state requires current official-source evidence and re-verification due date.';
COMMENT ON TABLE private.supplier_commerce_retention_registry IS 'Supplier Commerce privacy/retention registry framework; actual retention rules must be populated and verified before affected capabilities activate.';

DO $$
BEGIN
  IF to_regprocedure('public.server_supplier_commerce_control_decision_v1(text,jsonb)') IS NULL THEN RAISE EXCEPTION 'Supplier Commerce control decision RPC missing'; END IF;
  IF to_regprocedure('public.server_set_supplier_commerce_control_v1(uuid,text,text,text,boolean,text)') IS NULL THEN RAISE EXCEPTION 'Supplier Commerce control mutation RPC missing'; END IF;
  IF NOT EXISTS (SELECT 1 FROM private.supplier_commerce_controls WHERE operation='*' AND scope_type='global' AND enabled=false) THEN RAISE EXCEPTION 'Supplier Commerce global safe-default control missing'; END IF;
END;
$$;;
