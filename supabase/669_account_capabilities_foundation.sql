-- 669_account_capabilities_foundation.sql
...
BEGIN
  ...
  ELSIF NEW.role = 'admin' THEN
    ...
    -- commerce capability. Revoke ordinary grants immediately.
...
-- Explicitly reconcile Admin rows in case a prior environment already had
-- ordinary grants before this migration was introduced.
...
BEGIN
  ...
  -- A draft/submitted/suspended Seller relationship must never inherit an
  -- accidentally active store. Existing fully-active sellers remain unchanged
  -- when this idempotent boundary is called again.
  INSERT INTO public.seller_stores ("userId", "isActive")
  VALUES (p_user_id, false)
  ON CONFLICT ("userId") DO UPDATE
    SET "isActive" = false
    WHERE COALESCE(v_existing_status, 'draft') <> 'active';
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION '669 capability backfill failed: Buyer without Buyer capability';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '669 capability backfill failed: Seller missing Buyer/Seller capability';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '669 capability isolation failed: Admin retains active ordinary capability';
  END IF;
END $$;
