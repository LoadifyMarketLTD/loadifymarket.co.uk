...
--
-- This migration does NOT switch public registration yet.
-- It creates the private server-owned intent store and fixes the ongoing
-- email-confirmation projection from auth.users -> public.users.
--
-- Invariants:
--   * passwords are NEVER stored here
--   * client roles have zero direct access
...
CREATE TABLE IF NOT EXISTS private.signup_intents (
  ...
  seller_type text NULL
    CHECK (
      seller_type IS NULL
      OR seller_type IN ('individual', 'sole_trader', 'company')
    ),
  ...
  CONSTRAINT signup_intents_email_normalized
    CHECK (email = lower(btrim(email))),

  CONSTRAINT signup_intents_expiry_after_creation
    CHECK (expires_at > created_at),

  CONSTRAINT signup_intents_seller_type_contract
    CHECK (
...
GRANT SELECT, INSERT, UPDATE, DELETE
  ON TABLE private.signup_intents
  TO service_role;
...
BEGIN
  ...
  IF v_email IS NULL OR v_email = '' THEN
    RAISE EXCEPTION 'invalid signup intent email';
  END IF;
...
REVOKE ALL ON FUNCTION public.create_signup_intent(
  text,
  text,
  text,
  text,
...
GRANT EXECUTE ON FUNCTION public.create_signup_intent(
  text,
  text,
  text,
  text,
...
BEGIN
  ...
  THEN
    ...
    WHERE id = NEW.id
      AND "isEmailVerified" IS DISTINCT FROM true;
  END IF;
...
REVOKE ALL ON FUNCTION private.sync_public_email_verified_from_auth()
  FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION private.sync_public_email_verified_from_auth()
  TO service_role;

DROP TRIGGER IF EXISTS trg_sync_public_email_verified_from_auth
  ON auth.users;
...
BEGIN
  IF has_table_privilege('anon', 'private.signup_intents', 'SELECT')
     ...
     OR has_table_privilege('authenticated', 'private.signup_intents', 'UPDATE')
  THEN
    RAISE EXCEPTION
      '676 signup intent security failure: client role has direct table access';
  END IF;

  IF has_function_privilege(
       'anon',
       'public.create_signup_intent(text,text,text,text,text,text,text,text,text,text,jsonb,timestamptz)',
       'EXECUTE'
     )
  ...
  THEN
    RAISE EXCEPTION
      '676 signup intent RPC security failure: client role can execute RPC';
  END IF;
  ...
  IF has_function_privilege(
       'anon',
       'private.sync_public_email_verified_from_auth()',
       'EXECUTE'
     )
     OR has_function_privilege(
       'authenticated',
  ...
  THEN
    RAISE EXCEPTION
      '676 email verification sync failure: trigger helper exposed to client';
  END IF;
END
$$;
