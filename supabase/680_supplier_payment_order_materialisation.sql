...
--
-- Marketplace commercial snapshot v1 remains valid and unchanged. Supplier-fulfilled
-- payments use a stricter v2 snapshot that requires a pre-created canonical order,
-- Stage 1/2 route truth, active Supplier Commerce reservation and shipping decision
-- references before a payment session may be inserted. This migration enables nothing.

-- Extend the existing generic order tax evidence columns without weakening the legacy
-- marketplace-v1 evidence contract.
...
BEGIN
  ...
  IF jsonb_typeof(v_buyer)<>'object'
     ...
     OR jsonb_typeof(v_buyer->'reverseCharge')<>'boolean'
  THEN RETURN false; END IF;
  ...
  IF jsonb_typeof(v_platform)<>'object'
     ...
     OR v_platform->>'returnResponsibility'<>'loadify'
  THEN RETURN false; END IF;

  IF jsonb_typeof(p_metadata->'items')<>'array'
     OR jsonb_array_length(p_metadata->'items')=0 THEN RETURN false; END IF;
...
END;
$$;
REVOKE ALL ON FUNCTION private.payment_session_has_supplier_snapshot_v2(jsonb)
  FROM PUBLIC,anon,authenticated,service_role;

-- Keep the existing trigger/function name so no deployment-order gap opens. New inserts
-- must satisfy either the unchanged marketplace-v1 contract or supplier-v2 contract.
...
CREATE OR REPLACE FUNCTION public.server_materialize_paid_supplier_order_v1(
...
BEGIN
  ...
  IF v_session.status NOT IN ('pending','completed') THEN
    RAISE EXCEPTION 'supplier payment session status % is not processable',v_session.status USING ERRCODE='P0001';
  END IF;
  IF upper(BTRIM(v_session.currency))<>'GBP' THEN
    RAISE EXCEPTION 'supplier payment session currency must be GBP' USING ERRCODE='22023';
  END IF;
  ...
  v_platform:=v_metadata->'platformSnapshot';
  v_tax:=v_metadata->'taxSnapshot';
  v_order_id:=(v_metadata->>'orderId')::uuid;
  ...
  v_tax_pence:=(v_metadata->>'taxAmountPence')::bigint;
  ...
  IF v_total_pence<=0 OR v_shipping_pence<0 OR v_tax_pence<0
     ...
     OR v_session."userId" IS DISTINCT FROM v_buyer_id
     OR v_session."orderId" IS DISTINCT FROM v_order_id
  ...
  IF v_order."buyerId" IS DISTINCT FROM v_buyer_id
     OR v_order."sellerId" IS NOT NULL
  ...
  IF v_order.status NOT IN ('awaiting_payment','paid') THEN
    RAISE EXCEPTION 'pre-payment supplier order status % is not processable',v_order.status USING ERRCODE='P0001';
  END IF;
  IF v_order."stripePaymentIntentId" IS NOT NULL AND v_order."stripePaymentIntentId"<>p_payment_intent_id THEN
    RAISE EXCEPTION 'supplier order PaymentIntent identity cannot change' USING ERRCODE='P0001';
  END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(v_metadata->'items') LOOP
    ...
    SELECT * INTO v_order_item FROM public.order_items oi
     WHERE oi."orderId"=v_order_id AND oi."productId"=(v_item->>'productId')::uuid
     FOR UPDATE;
    IF NOT FOUND
       OR v_order_item.quantity IS DISTINCT FROM (v_item->>'quantity')::integer
    ...
    SELECT * INTO v_res FROM private.supplier_stock_reservations r
     WHERE r.id=(v_item->>'reservationId')::uuid
       AND r.order_id=v_order_id
       AND r.order_item_id=v_order_item.id
       AND r.supplier_offer_id=(v_item->>'supplierOfferId')::uuid
       AND r.pricing_snapshot_id=(v_item->>'pricingSnapshotId')::uuid
       AND r.status='active'
       AND r.expires_at>now()
     FOR UPDATE;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'active supplier reservation required at payment materialisation' USING ERRCODE='P0001';
    END IF;

    SELECT * INTO v_leg FROM private.supplier_fulfilment_legs l
     WHERE l.id=(v_item->>'fulfilmentLegId')::uuid
       AND l.orchestration_id=v_res.orchestration_id
       AND l.supplier_offer_id=v_res.supplier_offer_id
       AND l.commercial_mode='loadify_supplier_fulfilled';
    IF NOT FOUND THEN
      RAISE EXCEPTION 'supplier fulfilment leg identity mismatch at payment materialisation' USING ERRCODE='P0001';
    END IF;
  END LOOP;
  ...
  IF v_order.status='awaiting_payment' THEN
    UPDATE public.orders SET
      status='paid',"stripePaymentIntentId"=p_payment_intent_id,"updatedAt"=now()
  ...
  UPDATE public.payment_sessions SET
    "stripePaymentIntent"=p_payment_intent_id,status='completed',amount=v_total,"updatedAt"=now()
  WHERE id=v_session.id;

  RETURN jsonb_build_object(
    'orderId',v_order.id,'orderNumber',v_order."orderNumber",
    'commercialMode','loadify_supplier_fulfilled','sellerId',NULL,
    'customerTotal',v_total,'firstPaidTransition',v_first_paid_transition,'interfaceVersion',1
  );
END;
$$;
REVOKE ALL ON FUNCTION public.server_materialize_paid_supplier_order_v1(uuid,text)
  FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.server_materialize_paid_supplier_order_v1(uuid,text)
  TO service_role;

COMMENT ON FUNCTION private.payment_session_has_supplier_snapshot_v2(jsonb) IS
  'Stage 4B fail-closed supplier payment metadata contract. It requires canonical order, route, reservation and shipping decision references before payment-session creation.';
COMMENT ON FUNCTION public.server_materialize_paid_supplier_order_v1(uuid,text) IS
  'Stage 4B atomic paid transition for an already-prepared supplier-fulfilled canonical order. It does not submit a supplier order; Phase J remains a separate downstream handshake.';
