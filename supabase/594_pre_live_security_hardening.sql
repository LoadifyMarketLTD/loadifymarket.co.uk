...
-- Connect sellers are paid via Stripe transfers. Never also add the same order
-- to their withdrawable internal balance; doing both would create a double-payout
-- path if Connect is later disabled.
...
DECLARE
  ...
  v_connect_status text;
BEGIN
  SELECT "sellerId", subtotal, commission
    INTO v_order_seller, v_subtotal, v_commission
    FROM public.orders
   WHERE id = p_order_id;
