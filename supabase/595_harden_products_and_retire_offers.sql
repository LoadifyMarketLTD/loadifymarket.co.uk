...
-- Offers: remove the public/authenticated surface now that the marketplace uses
-- fixed seller prices. Tables may remain temporarily for historical cleanup, but
-- no authenticated client can read/insert offers through RLS and no RPC remains.
...
DO $$
BEGIN
  RAISE NOTICE '595_harden_products_and_retire_offers: product writes hardened and offer RPCs retired.';
END $$;
