...
-- Phase D — Supplier Foundation: Adapter, Qualification, SLA, Compliance, Provenance.
-- This migration does not enable Supplier Commerce. Phase C controls remain authoritative.
...
CREATE TABLE IF NOT EXISTS private.supplier_foundation_suppliers (
  ...
  CONSTRAINT supplier_foundation_supplier_key_check CHECK (
    supplier_key = lower(BTRIM(supplier_key))
    AND supplier_key ~ '^[a-z0-9][a-z0-9._-]{2,127}$'
  ),
  CONSTRAINT supplier_foundation_warehouse_refs_check CHECK (jsonb_typeof(warehouse_refs) = 'array'),
  CONSTRAINT supplier_foundation_lifecycle_check CHECK (
    lifecycle_status IN ('candidate','verification','approved','restricted','suspended','banned')
  ),
  CONSTRAINT supplier_foundation_approved_check CHECK (
    lifecycle_status <> 'approved' OR (approved_at IS NOT NULL AND approved_by IS NOT NULL)
  )
);
...
CREATE TABLE IF NOT EXISTS private.supplier_qualification_evidence (
  ...
  CONSTRAINT supplier_qualification_status_check CHECK (
    status IN ('unverified','verified','rejected','stale','not_applicable')
  ),
  CONSTRAINT supplier_qualification_verified_check CHECK (
    status <> 'verified'
...
CREATE TABLE IF NOT EXISTS private.supplier_sla_versions (
  ...
  CONSTRAINT supplier_sla_active_approval_check CHECK (
...
CREATE UNIQUE INDEX IF NOT EXISTS supplier_sla_one_active_unique
  ON private.supplier_sla_versions(supplier_id) WHERE status = 'active';
...
CREATE TABLE IF NOT EXISTS private.supplier_compliance_profiles (
  ...
  CONSTRAINT supplier_compliance_status_check CHECK (
    status IN ('unreviewed','approved','manual_review','prohibited','stale')
  ),
  ...
  CONSTRAINT supplier_compliance_prohibited_check CHECK (
    risk_class <> 'red' OR status IN ('prohibited','manual_review','stale')
  )
);
...
CREATE TABLE IF NOT EXISTS private.supplier_provenance_records (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT supplier_provenance_entity_check CHECK (
    entity_type IN ('supplier','catalog','product','variant','asset','document')
  ),
  CONSTRAINT supplier_provenance_rights_check CHECK (
    rights_status IN ('unknown','verified','restricted','prohibited')
  ),
  CONSTRAINT supplier_provenance_transform_check CHECK (
    transformation_status IN ('original','normalized','transformed')
  ),
  CONSTRAINT supplier_provenance_review_check CHECK (
    review_status IN ('unreviewed','approved','manual_review','rejected')
  ),
...
CREATE TABLE IF NOT EXISTS private.supplier_adapter_registrations (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT supplier_adapter_status_check CHECK (
    status IN ('disabled','verification','active','deprecated','blocked')
  ),
  CONSTRAINT supplier_adapter_capabilities_check CHECK (
    capabilities <@ ARRAY[
      'supplier_identity','catalog','variants','stock','price','shipping','order_submission',
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'eligible', false, 'reason', 'active_sla_missing',
  ...
  IF NOT FOUND OR v_compliance.status <> 'approved' OR v_compliance.risk_class = 'red'
    ...
    RETURN jsonb_build_object(
      'eligible', false, 'reason', 'compliance_not_approved',
      'supplierId', v_supplier.id, 'interfaceVersion', 1
    );
  END IF;
  ...
  IF NULLIF(lower(BTRIM(p_required_capability)), '') IS NOT NULL
    ...
    RETURN jsonb_build_object(
      'eligible', false, 'reason', 'adapter_capability_missing',
      'supplierId', v_supplier.id, 'interfaceVersion', 1
    );
  END IF;
...
BEGIN
  ...
  IF v_action = 'upsert_supplier' THEN
    ...
    SELECT * INTO v_supplier
      FROM private.supplier_foundation_suppliers
     WHERE supplier_key = lower(BTRIM(v_payload ->> 'supplierKey'))
     FOR UPDATE;

    IF FOUND AND v_supplier.lifecycle_status IN ('approved','restricted','suspended','banned') THEN
      RAISE EXCEPTION 'material supplier identity changes require verification lifecycle' USING ERRCODE = '23514';
    END IF;
    ...
    RETURN jsonb_build_object(
      'ok', true, 'supplierId', v_supplier.id,
      'supplierKey', v_supplier.supplier_key, 'interfaceVersion', 1
    );
  END IF;
  ...
  IF v_action = 'set_lifecycle' THEN
    ...
    INSERT INTO private.supplier_foundation_lifecycle_audit(
      ...
      v_supplier.id, p_actor_id, v_previous, v_next,
      v_supplier.lifecycle_version, v_next_version, v_reason
    );

    RETURN jsonb_build_object(
      'ok', true, 'supplierId', v_supplier.id, 'status', v_next,
      'lifecycleVersion', v_next_version, 'interfaceVersion', 1
    );
  END IF;
  ...
  IF v_action = 'activate_sla' THEN
    ...
    UPDATE private.supplier_sla_versions
       SET status = 'superseded', effective_to = LEAST(COALESCE(effective_to, now()), now())
    ...
    RETURN jsonb_build_object(
      'ok', true, 'supplierId', v_supplier.id,
      'slaVersion', v_sla_version, 'interfaceVersion', 1
    );
  END IF;
  ...
  IF v_action = 'record_provenance' THEN
    IF NULLIF(BTRIM(v_payload ->> 'entityType'), '') IS NULL
       OR NULLIF(BTRIM(v_payload ->> 'externalRef'), '') IS NULL
       OR NULLIF(BTRIM(v_payload ->> 'sourceRef'), '') IS NULL THEN
      RAISE EXCEPTION 'entityType, externalRef and sourceRef are required' USING ERRCODE = '22023';
    END IF;
  ...
  IF v_action = 'register_adapter' THEN
    IF NULLIF(BTRIM(v_payload ->> 'providerKey'), '') IS NULL
       OR NULLIF(BTRIM(v_payload ->> 'adapterKey'), '') IS NULL
       OR NULLIF(BTRIM(v_payload ->> 'adapterVersion'), '') IS NULL THEN
      RAISE EXCEPTION 'providerKey, adapterKey and adapterVersion are required' USING ERRCODE = '22023';
    END IF;
    ...
    INSERT INTO private.supplier_adapter_registrations(
      ...
      COALESCE(NULLIF(v_payload ->> 'interfaceVersion', '')::integer, 1),
    ...
    ON CONFLICT (supplier_id, provider_key, adapter_key, interface_version, adapter_version)
    DO UPDATE SET
      status = EXCLUDED.status,
