-- 613_seller_tax_declaration_evidence.sql
--
-- P1 seller-side evidence capture.
--
-- seller_profiles.isVatRegistered historically defaulted to false, therefore a
-- false value by itself is NOT proof that the seller explicitly declared a tax
-- status. This migration adds an explicit confirmation flag plus server-derived
-- version/source/timestamp evidence. Existing rows stay unconfirmed.
...
BEGIN
  -- service_role/admin maintenance may preserve already-derived evidence, but
  -- browser/mobile authenticated sellers can never choose source/version/time.
