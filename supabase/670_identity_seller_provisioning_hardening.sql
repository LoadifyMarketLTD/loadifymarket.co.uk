...
-- The historical profile trigger relied on seller_stores.isActive defaulting
-- true and on the old sellerStatus default. That is unsafe for progressive
-- Seller onboarding. Keep the same trigger name/signature but make its inserts
...
-- Reconcile capability rows for Admin accounts that may pre-date this hardening.
...
-- Historical seller_stores rows may have inherited isActive=true from the old
-- table default even when the Seller account was draft/submitted/suspended or
-- no longer a Seller. Fail those rows closed without touching a genuinely
-- active Seller relationship.
