...
describe('canonical shipment write boundary', () => {
  ...
  it('suppresses duplicate user-facing side effects on an idempotent status retry', async () => {
    const notificationInsert = vi.fn(); const fetchMock = vi.fn(); vi.stubGlobal('fetch', fetchMock);
    ...
    expect(res.statusCode).toBe(200); expect(JSON.parse(res.body).changed).toBe(false); expect(notificationInsert).not.toHaveBeenCalled(); expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it('requires shipped payment evidence for Out for Delivery', async () => {
    ...
    expect(res.statusCode).toBe(200); expect(paymentGuard).toHaveBeenCalledWith(expect.objectContaining({ nextStatus: 'shipped' }));
  });

  it('does not mint a new POD upload URL after canonical proof is attached', async () => {
    const createSignedUploadUrl = vi.fn();
    const supabase = {
      auth: { getUser: vi.fn().mockResolvedValue({ data: { user: { id: 'seller-1' } }, error: null }) }, rpc: vi.fn(), storage: { from: vi.fn(() => ({ createSignedUploadUrl })) },
      from: vi.fn((table: string) => {
    ...
    expect(res.statusCode).toBe(409); expect(createSignedUploadUrl).not.toHaveBeenCalled();
  });

  it('treats confirmation of the exact canonical POD path as an idempotent success', async () => {
    const filePath = 'shipment-1/shipment-1-123.jpg'; const rpc = vi.fn(); const remove = vi.fn();
    const supabase = {
      auth: { getUser: vi.fn().mockResolvedValue({ data: { user: { id: 'seller-1' } }, error: null }) }, rpc, storage: { from: vi.fn(() => ({ remove })) },
      from: vi.fn((table: string) => {
        if (table === 'users') return { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), maybeSingle: vi.fn().mockResolvedValue({ data: { id: 'seller-1', role: 'seller', isActive: true }, error: null }) };
    ...
    expect(res.statusCode).toBe(200); expect(JSON.parse(res.body).attached).toBe(false); expect(rpc).not.toHaveBeenCalled(); expect(remove).not.toHaveBeenCalled();
  });

  it('removes an uploaded POD object when the atomic DB attachment is rejected', async () => {
    const remove = vi.fn().mockResolvedValue({ error: null }); const filePath = 'shipment-1/shipment-1-123.jpg';
    ...
    const supabase = {
      auth: { getUser: vi.fn().mockResolvedValue({ data: { user: { id: 'seller-1' } }, error: null }) }, rpc, storage: { from: vi.fn(() => storageApi) },
      from: vi.fn((table: string) => {
        if (table === 'users') return { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), maybeSingle: vi.fn().mockResolvedValue({ data: { id: 'seller-1', role: 'seller', isActive: true }, error: null }) };
