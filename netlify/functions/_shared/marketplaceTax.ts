...
export type MarketplaceTaxDecision =
  | {
      ok: true;
      snapshot: MarketplaceTaxDecisionSnapshot;
...
function addressCountry(address: Record<string, string> | undefined): string | null {
  ...
  return normaliseCountry(address.countryCode ?? address.country ?? address.country_code);
}
...
/**
 * P1 marketplace tax boundary.
 *
 * This resolver intentionally supports only the transaction class that can be
 * proved from the current marketplace evidence without inventing VAT rules:
 *
 *   GB-established seller (from the canonical seller profile)
 *   + seller explicitly not VAT registered
 *   + physical product carrying canonical non-VAT tax evidence
 *   + GB delivery/billing destination.
 *
 * VAT-registered sellers, services, international destinations and every
 * incomplete/contradictory evidence state fail closed until the versioned tax
 * engine in the Supplier Commerce programme authorises those transaction classes.
 * A VAT-verified B2B buyer does NOT by itself create a reverse-charge result.
 */
export function resolveMarketplaceTaxV1(input: {
  ...
  if (normaliseCountry(input.seller.country) !== 'GB') {
    return {
      ok: false,
  ...
  for (const product of input.products) {
    ...
    const evidenceIsComplete =
      product.taxTreatmentStatus === MARKETPLACE_NON_VAT_TREATMENT &&
      product.taxEvidenceVersion === MARKETPLACE_TAX_EVIDENCE_VERSION &&
      Boolean(product.taxTreatmentSource?.trim()) &&
