...
async function executeBinding(
  ...
  if (binding.transport !== 'http_rest' && binding.transport !== 'graphql') {
    return { ok: false, errorClass: 'CAPABILITY_UNAVAILABLE', message: 'Supplier transport is not executable by the HTTP/GraphQL runtime' };
  }
  ...
  try {
    ...
    if (!response.ok) return normalizeError(response.status);

    const declared = Number(response.headers.get('content-length') ?? 0);
    if (Number.isFinite(declared) && declared > (config.maxBytes ?? 2 * 1024 * 1024)) {
      return { ok: false, errorClass: 'MALFORMED_RESPONSE', message: 'Supplier runtime response exceeds configured limit' };
    }
    const raw = await response.text();
    if (Buffer.byteLength(raw, 'utf8') > (config.maxBytes ?? 2 * 1024 * 1024)) {
      return { ok: false, errorClass: 'MALFORMED_RESPONSE', message: 'Supplier runtime response exceeds configured limit' };
    }
    let payload: unknown;
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return { ok: false, errorClass: 'RETRYABLE_FAILURE', message: 'Supplier runtime request timed out' };
    }
    return { ok: false, errorClass: 'UNKNOWN_OUTCOME', message: error instanceof Error ? error.message : 'Supplier runtime request failed' };
  } finally {
