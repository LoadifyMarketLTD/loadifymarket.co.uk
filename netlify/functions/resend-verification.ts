...
async function getAuthUser(
  ...
  if (error || !user) {
    console.error('resend-verification: auth.getUser failed:', error?.message ?? 'no user returned');
    return null;
  }
  ...
  if (dbError) {
    console.error('resend-verification: users table lookup failed:', dbError.message);
  }
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (!caller) {
      return { statusCode: 401, body: JSON.stringify({ error: 'Unauthorized - valid admin session required' }) };
    }
    ...
    if (targetErr || !targetUser?.email) {
      console.error('resend-verification: target user lookup failed:', targetErr?.message);
    ...
    if (linkErr || !linkData) {
      const errMsg = linkErr?.message ?? 'no link data returned';
      console.error('resend-verification: generateLink failed:', errMsg, linkErr);
      return {
        statusCode: 500,
        body: JSON.stringify({ error: `Failed to generate verification link: ${errMsg}` }),
      };
    }
    ...
    if (!actionLink) {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'Failed to generate verification link: action_link missing' }),
      };
    }
