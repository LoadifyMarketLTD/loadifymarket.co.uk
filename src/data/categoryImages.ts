 ...
 *
 * Reserve proved the robust pattern: root navigation images are local/bundled,
 * not third-party hotlinks. The primary repo already owns a same-origin library
 * under public/images/categories, so use that library here.
 */
...
export const categoryImages: Record<string, string> = {
  ...
  // Dedicated files for these six families are not yet present in the primary
  // repository. Keep the source same-origin and visually safe until the exact
  // Reserve binaries are ported; never fall back to a broken remote image.
