    ...
    <Table>
      <TableHeader>
        <TableRow style={{ borderBottom: "1px solid rgba(148,163,184,0.3)" }}>
          ...
          <TableHead className="hidden sm:table-cell text-xs font-semibold tracking-wide uppercase" style={{ color: "rgba(71,85,105,0.8)" }}>Reported By</TableHead>
        ...
        {loading ? (
          ...
          data.map((f) => (
            <TableRow key={f.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <TableCell>
                <p className="text-sm font-medium text-slate-900">{f.productTitle}</p>
                <p className="text-xs" style={{ color: "rgba(71,85,105,0.85)" }}>{f.date}</p>
              </TableCell>
              <TableCell className="hidden sm:table-cell text-xs" style={{ color: "rgba(71,85,105,0.85)" }}>{f.reportedBy}</TableCell>
              <TableCell className="hidden md:table-cell text-xs max-w-[150px] truncate" style={{ color: "rgba(71,85,105,0.85)" }}>{f.reason}</TableCell>
              <TableCell>
              ...
              <TableCell className="text-right">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-slate-900 hover:bg-white/10" disabled={actionLoading === f.id}>
                      {actionLoading === f.id ? (
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6" style={{ background: "#f8fafc", minHeight: "100%" }}>
      <div className="pb-2" style={{ borderBottom: "1px solid rgba(148,163,184,0.3)" }}>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Flagged Content</h1>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span style={{ color: "rgba(71,85,105,0.8)" }}>Reported By</span><p className="font-medium text-slate-900">{selected.reportedBy}</p></div>
                <div><span style={{ color: "rgba(71,85,105,0.8)" }}>Reason</span><p className="font-medium text-slate-900">{selected.reason}</p></div>
              ...
              <div className="rounded-xl p-3" style={{ background: "#ffffff", border: "1px solid rgba(148,163,184,0.35)" }}>
                <p className="text-xs font-semibold mb-1" style={{ color: "rgba(71,85,105,0.8)" }}>DETAILS</p>
