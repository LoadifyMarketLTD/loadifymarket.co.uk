...
export default function SellerCoupons(){
  ...
  return <div className="p-4 sm:p-6 space-y-6"><div><h1 className="text-2xl font-bold">Coupons</h1><p className="text-sm text-muted-foreground">Create seller-specific promo codes for checkout.</p></div>
    <Card><CardHeader><CardTitle className="text-base">New coupon</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-5"><Input placeholder="CODE" value={code} onChange={e=>setCode(e.target.value)} /><select className="h-10 rounded-md border bg-background px-3 text-sm" value={discountType} onChange={e=>setDiscountType(e.target.value)}><option value="percentage">Percentage</option><option value="fixed_amount">Fixed amount</option><option value="free_shipping">Free shipping</option></select><Input type="number" min="0" value={value} onChange={e=>setValue(e.target.value)} placeholder="Value"/><Input type="number" min="0" value={minOrder} onChange={e=>setMinOrder(e.target.value)} placeholder="Minimum order"/><Button onClick={createCoupon} disabled={saving}>{saving?"Creating…":"Create"}</Button></CardContent></Card>
    <div className="space-y-3">{rows.length===0?<Card><CardContent className="py-8 text-center text-muted-foreground">No coupons yet.</CardContent></Card>:rows.map(c=><Card key={c.id}><CardContent className="flex flex-wrap items-center justify-between gap-3 py-4"><div><div className="font-bold">{c.code}</div><div className="text-sm text-muted-foreground">{c.discountType==="percentage"?`${c.discountValue}%`:c.discountType==="fixed_amount"?`£${c.discountValue.toFixed(2)}`:"Free shipping"} · used {c.usedCount}{c.maxUses?`/${c.maxUses}`:""}</div></div><Button variant={c.isActive?"outline":"default"} onClick={()=>toggle(c)}>{c.isActive?"Deactivate":"Activate"}</Button></CardContent></Card>)}</div>
  </div>;
}
