...
export default function BuyerSavedSearches() {
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    ...
    {loading?<p className="text-sm text-muted-foreground">Loadingâ€¦</p>:rows.length===0?<Card><CardContent className="py-10 text-center"><Search className="mx-auto mb-3 h-8 w-8 text-muted-foreground"/><p className="font-medium">No saved searches yet</p><p className="mt-1 text-sm text-muted-foreground">Search the catalog and use â€œSave searchâ€.</p></CardContent></Card>:<div className="grid gap-3">{rows.map(row=><Card key={row.id}><CardContent className="pt-5 flex flex-col gap-4 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="font-semibold truncate">{row.searchQuery||"Catalog search"}</p><p className="text-xs text-muted-foreground">{row.notificationFrequency} notifications Â· saved {new Date(row.createdAt).toLocaleDateString("en-GB")}</p></div><div className="flex gap-2"><Button size="sm" onClick={()=>open(row)}>Search</Button><Button size="sm" variant="outline" onClick={()=>void toggle(row)}>{row.emailNotifications?<Bell className="mr-1 h-4 w-4"/>:<BellOff className="mr-1 h-4 w-4"/>}{row.emailNotifications?"Alerts on":"Alerts off"}</Button><Button size="icon" variant="ghost" aria-label="Delete saved search" onClick={()=>void remove(row)}><Trash2 className="h-4 w-4"/></Button></div></CardContent></Card>)}</div>}
  </div>;
}
