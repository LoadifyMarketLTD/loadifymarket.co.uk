      ...
      <main id="main-content" className="bg-[#F8F7F4] text-[#0A234F]">
        <section className="border-b border-[#0A234F]/10">
          <div className="mx-auto grid max-w-[1480px] gap-12 px-5 py-16 sm:px-7 lg:grid-cols-12 lg:items-center lg:px-10 lg:py-24">
            <div className="lg:col-span-7">
              ...
              <div className="mt-8 flex flex-wrap gap-3"><Link to="/suppliers/apply" className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-[#0A234F] px-6 py-3 text-sm font-bold text-white hover:bg-[#071A3C]">Apply as a supplier <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link to="/integrations" className="inline-flex min-h-12 items-center rounded-lg border border-[#0A234F]/15 bg-white px-6 py-3 text-sm font-bold text-[#0A234F]">View technical routes</Link></div>
            </div>
        ...
        <section className="bg-white py-16 sm:py-20">
          <div className="mx-auto max-w-[1480px] px-5 sm:px-7 lg:px-10">
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{formats.map((format, index) => <article key={format} className="rounded-[24px] bg-[#0A234F] p-7 text-white shadow-[0_18px_50px_rgba(10,35,79,0.12)]"><div className="flex items-center justify-between"><FileInput className="h-6 w-6 text-[#F5A300]" aria-hidden="true" /><span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#F5A300]">{String(index + 1).padStart(2, "0")}</span></div><h3 className="mt-6 font-serif text-[1.8rem] font-normal leading-[1.08] tracking-[-0.03em] text-white">{format}</h3><p className="mt-4 text-[15px] leading-7 text-white/80">Supported as a governed catalogue transport when it matches the supplier's approved onboarding path.</p></article>)}</div>
          </div>
        </section>
