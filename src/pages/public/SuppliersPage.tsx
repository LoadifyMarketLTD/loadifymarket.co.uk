      ...
      <main id="main-content" className="bg-[#F8F7F4] text-[#0A234F]">
        <section className="border-b border-[#0A234F]/10 bg-[#F8F7F4]">
          <div className="mx-auto grid max-w-[1480px] gap-12 px-5 py-16 sm:px-7 lg:grid-cols-12 lg:items-center lg:px-10 lg:py-24">
            <div className="lg:col-span-7"><p className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-[#8A7351]">Suppliers, brands & wholesalers</p><h1 className="mt-5 max-w-[900px] font-serif text-[2.75rem] font-normal leading-[1.02] tracking-[-0.04em] sm:text-[3.8rem] lg:text-[4.5rem]">Bring products to a UK marketplace for brands, wholesalers and suppliers.</h1><p className="mt-7 max-w-[760px] text-[16px] leading-7 text-[#5A6578] sm:text-[18px] sm:leading-8">Sell through the marketplace, discuss supplier participation or explore controlled commerce connectivity. The right Loadify route depends on how your business provides products, catalogue data and fulfilment.</p><div className="mt-8 flex flex-wrap gap-3"><Link to="/contact?topic=supplier" className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-[#0A234F] px-6 py-3 text-sm font-bold text-white hover:bg-[#071A3C]">Supplier enquiry <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link to="/register?type=seller" className="inline-flex min-h-12 items-center rounded-lg border border-[#0A234F]/15 bg-white px-6 py-3 text-sm font-bold text-[#0A234F]">Sell products on Loadify</Link></div></div>
            <div className="lg:col-span-5"><div className="rounded-[26px] bg-[#0A234F] p-8 text-white shadow-[0_22px_65px_rgba(10,35,79,0.14)]"><Building2 className="h-7 w-7 text-[#F5A300]" aria-hidden="true" /><p className="mt-7 text-[10px] font-black uppercase tracking-[0.18em] text-[#F5A300]">SUPPLIER PARTICIPATION</p><h2 className="mt-3 font-serif text-3xl font-normal leading-[1.08] tracking-[-0.03em] text-white">Supplier and wholesale participation without blanket promises.</h2><p className="mt-5 text-[15px] leading-7 text-white/80">Supplier participation is assessed according to the commercial model, catalogue requirements, fulfilment responsibilities and any technical capabilities involved.</p><div className="mt-6 border-t border-white/15 pt-5 text-sm leading-6 text-white/75">Provider-specific automation is not presented as live unless the relevant capability has been independently verified and activated.</div></div></div>
          </div>
        </section>

        <section className="bg-white py-16 sm:py-20"><div className="mx-auto max-w-[1480px] px-5 sm:px-7 lg:px-10"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#8A7351]">Ways to work with Loadify</p><h2 className="mt-3 max-w-3xl font-serif text-3xl tracking-[-0.025em] sm:text-4xl">Choose the marketplace or supplier route that matches your operation.</h2><div className="mt-10 grid gap-5 lg:grid-cols-3">{paths.map(({ icon: Icon, eyebrow, title, copy, to, cta }) => <article key={title} className="flex min-h-[300px] flex-col rounded-[24px] bg-[#0A234F] p-8 text-white shadow-[0_18px_50px_rgba(10,35,79,0.12)]"><Icon className="h-7 w-7 text-[#F5A300]" aria-hidden="true" /><p className="mt-7 text-[10px] font-black uppercase tracking-[0.18em] text-[#F5A300]">{eyebrow}</p><h3 className="mt-3 font-serif text-[2rem] font-normal leading-[1.08] tracking-[-0.03em] text-white">{title}</h3><p className="mt-5 flex-1 text-[15px] leading-7 text-white/80">{copy}</p><Link to={to} className="mt-7 inline-flex items-center gap-2 text-sm font-extrabold text-white">{cta} <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></article>)}</div></div></section>
