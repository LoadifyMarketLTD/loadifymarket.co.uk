      ...
      <main id="main-content" className="bg-[#F8F7F4] text-[#0A234F]">
        ...
        <section className="border-y border-[#0A234F]/10 bg-[#F7F9FC] py-20">
          <div className="mx-auto max-w-[1480px] px-6 lg:px-10">
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-2">{supplierFlow.map(([step, copy], index) => <article key={step} className="flex min-h-[300px] flex-col rounded-[24px] bg-[#0A234F] p-8 text-white shadow-[0_18px_50px_rgba(10,35,79,0.12)] transition hover:-translate-y-0.5 hover:bg-[#071A3C] hover:shadow-[0_22px_56px_rgba(10,35,79,0.18)]"><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#F5A300]">{String(index + 1).padStart(2, "0")}</span>{index === 2 ? <ShieldCheck className="h-6 w-6 text-[#F5A300]" /> : index === 3 ? <Sparkles className="h-6 w-6 text-[#F5A300]" /> : index === 1 ? <Database className="h-6 w-6 text-[#F5A300]" /> : <PackageCheck className="h-6 w-6 text-[#F5A300]" />}</div><p className="mt-7 text-[10px] font-black uppercase tracking-[0.18em] text-[#F5A300]">SUPPLIER FLOW</p><h3 className="mt-3 font-serif text-[1.9rem] font-normal leading-[1.08] tracking-[-0.03em] text-white">{step}</h3><p className="mt-5 text-[15px] leading-7 text-white/80">{copy}</p></article>)}</div>
