      ...
      <main id="main-content" className="bg-[#F8F7F4] text-[#0A234F] md:pt-[122px]">
        <section className="border-b border-[#0A234F]/10">
          <div className="mx-auto grid max-w-[1480px] gap-12 px-5 py-16 sm:px-7 lg:grid-cols-12 lg:items-center lg:px-10 lg:py-24">
            <div className="lg:col-span-7">
              <p className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-[#8A7351]">Sell products online</p>
              <h1 className="mt-5 max-w-[900px] font-serif text-[2.75rem] font-normal leading-[1.02] tracking-[-0.04em] sm:text-[3.8rem] lg:text-[4.5rem]">Sell products online and manage your marketplace presence on Loadify.</h1>
              <p className="mt-7 max-w-[760px] text-[16px] leading-7 text-[#5A6578] sm:text-[18px] sm:leading-8">Create marketplace listings and use Seller Space to manage orders, shipments, returns, reviews, messages and account activity through one connected seller environment.</p>
              <div className="mt-8 flex flex-wrap gap-3"><Link to="/register?type=seller" className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-[#0A234F] px-6 py-3 text-sm font-bold text-white">Start selling online <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link to="/seller-guidelines" className="inline-flex min-h-12 items-center rounded-lg border border-[#0A234F]/15 bg-white px-6 py-3 text-sm font-bold text-[#0A234F]">Seller guidelines</Link></div>
            </div>
            <div className="lg:col-span-5">
              <div className="rounded-[26px] bg-[#0A234F] p-8 text-white shadow-[0_22px_65px_rgba(10,35,79,0.14)]">
                ...
                <h2 className="mt-3 font-serif text-3xl font-normal leading-[1.08] tracking-[-0.03em] text-white">Seller tools connected to your public marketplace presence.</h2>
        ...
        <section className="bg-white py-16 sm:py-20">
          <div className="mx-auto max-w-[1480px] px-5 sm:px-7 lg:px-10">
            ...
            <h2 className="mt-3 max-w-3xl font-serif text-3xl tracking-[-0.025em] sm:text-4xl">Core tools for day-to-day marketplace selling.</h2>
        ...
        <section className="bg-[#F7F9FC] py-16 sm:py-20">
          <div className="mx-auto grid max-w-[1480px] gap-10 px-5 sm:px-7 lg:grid-cols-[0.8fr_1.2fr] lg:px-10">
            <div><Settings className="h-6 w-6 text-[#8A7351]" /><h2 className="mt-5 font-serif text-3xl tracking-[-0.025em] sm:text-4xl">From seller registration to marketplace operations.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-[#667085]">The seller journey has distinct setup and operating stages. Registration alone should not be confused with completed marketplace readiness.</p></div>
        ...
        <section className="bg-[#0A234F] py-16 text-white sm:py-20"><div className="mx-auto max-w-[1180px] px-5 text-center sm:px-7"><Store className="mx-auto h-6 w-6 text-[#F5A300]" aria-hidden="true" /><h2 className="mx-auto mt-4 max-w-3xl font-serif text-3xl tracking-[-0.025em] sm:text-4xl">Ready to start selling products on Loadify?</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-white/70">Start the seller registration path, then complete the applicable setup and marketplace readiness requirements.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link to="/register?type=seller" className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-[#F5A300] px-6 py-3 text-sm font-extrabold text-[#0A234F]">Start selling <ArrowRight className="h-4 w-4" /></Link><Link to="/trust" className="inline-flex min-h-12 items-center rounded-lg border border-white/20 px-6 py-3 text-sm font-bold text-white">Trust & marketplace rules</Link></div></div></section>
      </main>
    </MainLayout>
  );
}
