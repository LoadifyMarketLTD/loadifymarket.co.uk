...
function OrderCard({ order, mode }: { order: OrderRow; mode: OrderMode }) {
  ...
  const openOrder = () => {
    navigate(`/orders?mode=${mode}&orderId=${encodeURIComponent(order.id)}`);
  };
...
function MobileOrderDetail({ orderId, requestedMode, onBack }: {
  orderId: string;
  requestedMode: OrderMode;
  onBack: () => void;
}) {
  const navigate = useNavigate();
    ...
    const load = async () => {
      ...
      try {
        const { data: orderData, error: orderError } = await supabase
          .from("orders")
          .select(`id, orderNumber, total, status, createdAt, quantity, buyerId, sellerId, buyerNameSnapshot, sellerBusinessNameSnapshot, commercialSnapshotSource, products:productId(title, images), order_items(productTitleSnapshot, productImageSnapshot, productSnapshotSource)`)
          .eq("id", orderId)
          .maybeSingle();
        ...
        const actualMode: OrderMode = requestedMode === "sell" && isSeller
          ? "sell"
          : requestedMode === "buy" && isBuyer
            ? "buy"
            : isSeller
              ? "sell"
              : "buy";
        ...
        let counterpartName = actualMode === "sell"
          ? raw.buyerNameSnapshot?.trim() || null
          : raw.sellerBusinessNameSnapshot?.trim() || null;
        ...
        const { data: returnRows } = await supabase
          .from("returns")
          .select("id, status, reason, refundAmount, createdAt")
          .eq("orderId", raw.id)
          .order("createdAt", { ascending: false })
          .limit(1);

        const snapshotItem = raw.order_items?.find((item) => item.productSnapshotSource != null) ?? null;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F7F9FC] px-[var(--mob-side,16px)] pt-8 md:hidden">
        <div className="h-12 animate-pulse rounded-[14px] bg-[#E8EDF3]" />
        <div className="mt-4 h-36 animate-pulse rounded-[20px] bg-[#E8EDF3]" />
        <div className="mt-3 h-48 animate-pulse rounded-[20px] bg-[#E8EDF3]" />
      </div>
    );
  }
  ...
  if (error || !detail) {
    return (
      <div className="min-h-screen bg-[#F7F9FC] px-[var(--mob-side,16px)] pt-8 text-[#0A234F] md:hidden">
        ...
        <div className="mt-6 rounded-[20px] border border-red-100 bg-white p-6 text-center shadow-sm">
          <AlertCircle className="mx-auto h-8 w-8 text-red-500" />
          <p className="mt-3 text-[14px] font-extrabold">{error ?? "Order unavailable"}</p>
        </div>
        <MobileBottomNav />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC] text-[#0A234F] md:hidden">
      <header
        className="sticky top-0 z-30 border-b border-[#0A234F]/[0.08] bg-white/95 px-[var(--mob-side,16px)] pb-3"
        style={{ paddingTop: "calc(0.7rem + env(safe-area-inset-top, 0px))", backdropFilter: "blur(18px)", WebkitBackdropFilter: "blur(18px)" }}
      >
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#C98200]">{mode === "sell" ? "Sale details" : "Order details"}</p>
            <p className="mt-0.5 truncate font-mono text-[12px] font-bold text-[#667085]">#{detail.orderNumber}</p>
          </div>
      ...
      <main className="space-y-3 px-[var(--mob-side,16px)] py-4" style={{ paddingBottom: "calc(92px + env(safe-area-inset-bottom, 0px))" }}>
        <section className="rounded-[20px] border border-[#0A234F]/[0.08] bg-white p-4 shadow-[0_7px_22px_rgba(10,35,79,0.05)]">
          <div className="flex gap-3.5">
            <div className="flex h-[88px] w-[88px] shrink-0 items-center justify-center overflow-hidden rounded-[15px] bg-[#EEF2F7]">
              {detail.productImage ? <img src={detail.productImage} alt={detail.productTitle ?? "Product"} className="h-full w-full object-cover" /> : <Package className="h-8 w-8 text-[#A0A8B4]" />}
            </div>
            <div className="min-w-0 flex-1">
              <p className="line-clamp-3 text-[14px] font-black leading-[1.35] text-[#26354A]">{detail.productTitle ?? "Order"}</p>
              <p className="mt-2 text-[11px] font-semibold text-[#667085]">{mode === "sell" ? "Buyer" : "Seller"}: {counterpart}</p>
              <div className="mt-2 flex items-end justify-between gap-2">
                <span className="text-[18px] font-black text-[#0A234F]">£{detail.total.toFixed(2)}</span>
                <span className="text-[10px] font-semibold text-[#7A8493]">Qty {detail.quantity}</span>
              </div>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 border-t border-[#0A234F]/[0.07] pt-3 text-[11px]">
            <div><p className="font-semibold text-[#98A2B3]">Order date</p><p className="mt-0.5 font-bold text-[#475569]">{formatDate(detail.createdAt)}</p></div>
            <div><p className="font-semibold text-[#98A2B3]">Order status</p><p className="mt-0.5 font-bold capitalize text-[#475569]">{cfg.label}</p></div>
          </div>
        </section>
          ...
          {detail.shipment ? (
            <>
              <div className="mt-3 rounded-[14px] bg-[#F7F9FC] p-3">
                <div className="flex items-center justify-between gap-2"><span className="text-[11px] font-semibold text-[#667085]">Shipment status</span><span className="text-[11px] font-extrabold capitalize text-[#0A234F]">{detail.shipment.status}</span></div>
              ...
              {detail.events.length > 0 ? (
                <div className="mt-4">
                  <p className="mb-3 text-[10px] font-black uppercase tracking-[0.12em] text-[#7A8493]">Delivery history</p>
                  <div className="space-y-3">
                    {detail.events.map((event, index) => (
                      <div key={event.id} className="flex gap-3">
                        <div className="flex flex-col items-center"><span className={`mt-1 h-2.5 w-2.5 rounded-full ${index === detail.events.length - 1 ? "bg-[#F5A300]" : "bg-[#B8C1CD]"}`} />{index < detail.events.length - 1 ? <span className="mt-1 h-full min-h-7 w-px bg-[#D9DEE6]" /> : null}</div>
                        <div className="min-w-0 flex-1 pb-1"><p className="text-[11px] font-extrabold capitalize text-[#26354A]">{event.status}</p>{event.message ? <p className="mt-0.5 text-[10px] leading-[1.45] text-[#667085]">{event.message}</p> : null}<p className="mt-1 text-[9px] font-medium text-[#98A2B3]">{formatDateTime(event.created_at)}</p></div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {detail.returnRequest ? (
          <section className="rounded-[20px] border border-[#0A234F]/[0.08] bg-white p-4 shadow-[0_7px_22px_rgba(10,35,79,0.05)]">
            ...
            <div className="mt-2 flex items-start justify-between gap-3">
              <div className="min-w-0"><p className="text-[13px] font-extrabold capitalize text-[#26354A]">{detail.returnRequest.status}</p><p className="mt-1 text-[10px] leading-[1.45] text-[#667085]">{detail.returnRequest.reason}</p></div>
        ...
        <section className="overflow-hidden rounded-[18px] border border-[#0A234F]/[0.08] bg-white shadow-[0_6px_22px_rgba(10,35,79,0.05)]">
          <button onClick={() => navigate("/inbox")} className="flex min-h-14 w-full items-center gap-3 px-4 text-left"><MessageSquare className="h-[18px] w-[18px] text-[#0A234F]" /><span className="flex-1 text-[13px] font-extrabold text-[#26354A]">Message {mode === "sell" ? "buyer" : "seller"}</span><ChevronRight className="h-4 w-4 text-[#A0A8B4]" /></button>
          <div className="ml-[52px] h-px bg-[#0A234F]/[0.07]" />
          <button onClick={() => navigate("/faq")} className="flex min-h-14 w-full items-center gap-3 px-4 text-left"><CircleHelp className="h-[18px] w-[18px] text-[#0A234F]" /><span className="flex-1 text-[13px] font-extrabold text-[#26354A]">Returns, refunds &amp; order help</span><ChevronRight className="h-4 w-4 text-[#A0A8B4]" /></button>
        </section>
      </main>
...
export default function MobileOrdersPage() {
  ...
  if (deepLinkOrderId) {
    return <MobileOrderDetail orderId={deepLinkOrderId} requestedMode={mode} onBack={closeDetail} />;
  }

  const visibleOrders = activeTab === "all"
    ? orders
    : orders.filter((order) => TAB_STATUSES[activeTab].includes(order.status));
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC] text-[#0A234F] md:hidden">
      ...
      >
        ...
        <div className="flex items-end justify-between gap-3">
          ...
          <div className="mb-0.5 flex rounded-full bg-[#EEF2F7] p-1">
            <button type="button" onClick={() => changeMode("buy")} className={`rounded-full px-3 py-1.5 text-[10px] font-extrabold ${mode === "buy" ? "bg-[#0A234F] text-white" : "text-[#667085]"}`}>Purchases</button>
            <button type="button" onClick={() => changeMode("sell")} className={`rounded-full px-3 py-1.5 text-[10px] font-extrabold ${mode === "sell" ? "bg-[#0A234F] text-white" : "text-[#667085]"}`}>Sales</button>
          </div>
        </div>
        <div className="mt-3 flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {TABS.map((tab) => {
            const active = activeTab === tab.id;
            return <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`min-h-9 shrink-0 rounded-full px-3 text-[11px] font-extrabold ${active ? "bg-[#0A234F] text-white" : "border border-[#0A234F]/10 bg-[#F7F9FC] text-[#667085]"}`}>{tab.label}</button>;
          })}
        </div>
      </header>
      ...
      <main className="px-[var(--mob-side,16px)] py-4" style={{ paddingBottom: "calc(88px + env(safe-area-inset-bottom, 0px))" }}>
        {loading ? (
          <div className="flex flex-col gap-3">{[1, 2, 3].map((n) => <div key={n} className="h-[108px] animate-pulse rounded-[18px] bg-[#E8EDF3]" />)}</div>
          ...
          <div className="flex flex-col items-center justify-center rounded-[20px] border border-[#0A234F]/[0.08] bg-white px-6 py-14 text-center shadow-sm">
            ...
            {activeTab === "all" && mode === "buy" ? <Link to="/catalog" className="mt-5 rounded-[13px] bg-[#0A234F] px-4 py-2.5 text-[12px] font-extrabold text-white no-underline">Browse marketplace</Link> : null}
          </div>
        ) : (
          <div className="flex flex-col gap-3">{visibleOrders.map((order) => <OrderCard key={order.id} order={order} mode={mode} />)}</div>
        )}
        ...
        {!loading ? (
          <button onClick={() => navigate("/faq")} className="mt-4 flex h-12 w-full items-center gap-2 rounded-[14px] border border-[#0A234F]/10 bg-white px-3 text-left">
            <CircleHelp className="h-4 w-4 shrink-0 text-[#667085]" aria-hidden="true" />
