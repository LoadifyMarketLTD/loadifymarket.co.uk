import { ShieldCheck, Star, BadgePercent, UserCheck, Wallet } from "lucide-react";
...
const TrustSection = () => {
  const items = [
    ...
    {
      icon: (
        <svg className="h-4 w-10 shrink-0" viewBox="0 0 60 25" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    },
    {
      ...
      label: "Rated 5.0 on Google",
    },
    {
      icon: <BadgePercent className="h-4 w-4 text-emerald-600 shrink-0" />,
      label: "0% Commission for New Sellers",
    },
    {
  ...
  return (
    <section className="border-y border-border bg-muted/50 py-4">
      <div className="container mx-auto px-4">
        <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-sm text-muted-foreground divide-x-0">
          {items.map((item, i) => (
