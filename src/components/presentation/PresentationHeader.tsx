...
export default function PresentationHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#0A234F]/10 bg-[#FCFBF8]/95 backdrop-blur-xl">
      ...
      {mobile && <div className="border-t border-[#0A234F]/10 bg-white px-6 py-5 xl:hidden"><div className="grid gap-1 sm:grid-cols-2">{navigation.map((item) => <Link onClick={() => setMobile(false)} key={item.to} to={item.to} className="rounded-lg px-3 py-3 text-sm font-bold text-[#0A234F] hover:bg-[#F8F7F4]">{item.label}</Link>)}</div><div className="mt-4 flex gap-2 border-t border-[#0A234F]/10 pt-4"><Link onClick={() => setMobile(false)} to="/marketplace" className="rounded-lg bg-[#F5A300] px-4 py-2.5 text-sm font-extrabold text-[#0A234F]">Marketplace</Link><Link onClick={() => setMobile(false)} to="/login" className="rounded-lg border px-4 py-2.5 text-sm font-bold">Sign in</Link></div></div>}
    </header>
  );
}
