  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#0A234F]/10 bg-[#FCFBF8]/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[82px] max-w-[1480px] items-center gap-7 px-6 lg:px-10">
        ...
        <button onClick={() => setMobile((value) => !value)} className="ml-1 rounded-lg p-2 text-[#0A234F] xl:hidden" aria-label={mobile ? "Close navigation" : "Open navigation"}>
          {mobile ? <X /> : <Menu />}
        </button>
      </div>

      {mobile && (
        <div className="border-t border-[#0A234F]/10 bg-white px-6 py-5 xl:hidden">
          <div className="grid gap-1 sm:grid-cols-2">
            {navigation.map((item) => (
              <Link onClick={() => setMobile(false)} key={item.to} to={item.to} className="rounded-lg px-3 py-3 text-sm font-bold text-[#0A234F] hover:bg-[#F8F7F4]">
                {item.label}
              </Link>
            ))}
          </div>
          <div className="mt-4 flex gap-2 border-t border-[#0A234F]/10 pt-4">
            <Link onClick={() => setMobile(false)} to="/marketplace" className="rounded-lg bg-[#F5A300] px-4 py-2.5 text-sm font-extrabold text-[#0A234F]">Marketplace</Link>
            <Link onClick={() => setMobile(false)} to="/login" className="rounded-lg border px-4 py-2.5 text-sm font-bold">Sign in</Link>
          </div>
        </div>
      )}
    </header>
