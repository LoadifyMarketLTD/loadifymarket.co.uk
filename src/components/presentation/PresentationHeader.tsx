 ...
 return <header className="fixed inset-x-0 top-0 z-50 border-b border-[#0A234F]/10 bg-[#FCFBF8]/95 backdrop-blur-xl">
  <div className="mx-auto flex h-[82px] max-w-[1480px] items-center gap-7 px-6 lg:px-10">
   ...
   <nav className="hidden items-center gap-6 xl:flex" aria-label="Corporate navigation">
    ...
    <div className="relative"><button onClick={()=>{setBusiness(v=>!v);setTechnology(false)}} className="flex items-center gap-1 text-[13px] font-bold text-[#0A234F]">Business <ChevronDown className="h-3.5 w-3.5"/></button>{business&&<div className="absolute left-0 top-8 w-72 rounded-2xl border border-[#0A234F]/10 bg-white p-2 shadow-xl"><Link onClick={()=>setBusiness(false)} to="/trade" className="block rounded-xl px-4 py-3 hover:bg-[#F8F7F4]"><b className="text-sm">Trade Buyers</b><p className="mt-1 text-xs text-[#667085]">Business buying through Loadify</p></Link><Link onClick={()=>setBusiness(false)} to="/suppliers" className="block rounded-xl px-4 py-3 hover:bg-[#F8F7F4]"><b className="text-sm">Suppliers, Brands & Wholesalers</b><p className="mt-1 text-xs text-[#667085]">Explore controlled participation paths</p></Link></div>}</div>
    <div className="relative"><button onClick={()=>{setTechnology(v=>!v);setBusiness(false)}} className="flex items-center gap-1 text-[13px] font-bold text-[#0A234F]">Technology / Integrations <ChevronDown className="h-3.5 w-3.5"/></button>{technology&&<div className="absolute left-0 top-8 w-64 rounded-2xl border border-[#0A234F]/10 bg-white p-2 shadow-xl"><Link onClick={()=>setTechnology(false)} to="/integrations" className="block rounded-xl px-4 py-3 hover:bg-[#F8F7F4]"><b className="text-sm">Integrations</b><p className="mt-1 text-xs text-[#667085]">Controlled connectivity paths</p></Link><Link onClick={()=>setTechnology(false)} to="/developers" className="block rounded-xl px-4 py-3 hover:bg-[#F8F7F4]"><b className="text-sm">Developers</b><p className="mt-1 text-xs text-[#667085]">Technical participation context</p></Link></div>}</div>
  ...
  {mobile&&<div className="border-t border-[#0A234F]/10 bg-white px-6 py-5 xl:hidden"><div className="grid gap-1 sm:grid-cols-2">{[...direct,{label:"Trade Buyers",to:"/trade"},{label:"Suppliers, Brands & Wholesalers",to:"/suppliers"},{label:"Integrations",to:"/integrations"},{label:"Developers",to:"/developers"}].map(x=><Link onClick={()=>setMobile(false)} key={x.to} to={x.to} className="rounded-lg px-3 py-3 text-sm font-bold text-[#0A234F] hover:bg-[#F8F7F4]">{x.label}</Link>)}</div><div className="mt-4 flex gap-2 border-t border-[#0A234F]/10 pt-4"><Link onClick={()=>setMobile(false)} to="/marketplace" className="rounded-lg bg-[#F5A300] px-4 py-2.5 text-sm font-extrabold text-[#0A234F]">Marketplace</Link><Link onClick={()=>setMobile(false)} to="/login" className="rounded-lg border px-4 py-2.5 text-sm font-bold">Sign in</Link></div></div>}
 </header>
}
