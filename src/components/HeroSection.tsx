      ...
      <div className="relative mx-auto grid min-h-[760px] w-full max-w-[1280px] grid-cols-1 items-center gap-12 px-4 pb-16 pt-[164px] sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:gap-14 lg:px-10 lg:pb-20 lg:pt-[184px]">
        <div className="max-w-[650px]">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <Link to="/catalog" data-magnetic className="inline-flex min-h-12 items-center justify-center rounded-xl border border-[#0A234F]/20 bg-white px-6 py-3 text-sm font-bold text-[#0A234F] shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[#0E3FA9]/40 hover:text-[#0E3FA9]">
              Shop live products
            </Link>
          </div>
          ...
          <div className="mt-7 flex flex-wrap gap-x-6 gap-y-3 border-t border-[#0A234F]/10 pt-5 text-sm font-semibold text-[#334155]">
            <span className="inline-flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-[#0E3FA9]" aria-hidden="true" />Stripe-powered checkout</span>
            <span className="inline-flex items-center gap-2"><Truck className="h-4 w-4 text-[#0E3FA9]" aria-hidden="true" />Order tracking</span>
            <span className="inline-flex items-center gap-2"><Store className="h-4 w-4 text-[#0E3FA9]" aria-hidden="true" />UK operated</span>
          </div>
        </div>
        ...
        <div className="relative mx-auto w-full max-w-[620px] lg:mx-0">
          ...
          <div className="relative overflow-hidden rounded-[30px] border border-[#0A234F]/10 bg-white p-3 shadow-[0_30px_80px_rgba(10,35,79,0.16)] sm:p-4">
            <div className="mb-3 flex items-center justify-between px-1 sm:mb-4">
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.16em] text-[#0E3FA9]">Live on Loadify</p>
                <p className="mt-1 text-sm font-extrabold text-[#0A234F]">Real listings from the marketplace</p>
              </div>
              <Link to="/catalog" className="text-xs font-extrabold text-[#0E3FA9] hover:text-[#0A234F]">Browse all</Link>
            </div>
                ...
                {liveProducts[0] && (
                  <Link to={`/product/${liveProducts[0].id}`} className="group relative flex h-[390px] flex-col overflow-hidden rounded-[22px] bg-[#F1F4F8]">
                    ...
                    <div className="bg-white p-4">
                      <p className="line-clamp-1 text-[9px] font-black uppercase tracking-[0.14em] text-[#0E3FA9]">{liveProducts[0].category}</p>
                ...
                <div className="grid gap-3">
                  {liveProducts.slice(1, 3).map((product) => (
                    <Link key={product.id} to={`/product/${product.id}`} className="group relative flex h-[188px] overflow-hidden rounded-[22px] bg-[#F1F4F8]">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-[#071B3A]/88 via-[#071B3A]/10 to-transparent" aria-hidden="true" />
