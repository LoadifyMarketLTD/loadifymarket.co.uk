...
const Header = () => {
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 z-40 hidden border-b border-white/10 bg-[#0A234F] shadow-[0_8px_25px_rgba(10,35,79,0.28)] md:block"
    ...
    >
      <div className="w-full px-4 sm:px-6 lg:px-8 h-[60px] md:h-[72px] flex items-center gap-4">
        <button
          className="p-2.5 text-white/75 bg-white/[0.07] hover:text-[#F5A300] hover:bg-white/[0.12] active:bg-white/[0.16] rounded-xl transition-all shrink-0 ring-1 ring-white/15"
        ...
        <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-2xl mx-auto min-w-0" role="search">
          <div className="relative flex items-center w-full">
            ...
            <input
              ...
              className="w-full h-11 sm:h-[46px] pl-9 sm:pl-11 pr-10 sm:pr-28 bg-white/[0.07] border border-white/15 rounded-2xl text-xs sm:text-sm text-white placeholder:text-white/55 focus:outline-none focus:border-[#F5A300]/50 focus:ring-2 focus:ring-[#F5A300]/15 transition-all duration-200"
            />
            <button
              type="submit"
              className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 sm:h-[34px] px-3 sm:px-5 bg-[#F5A300] hover:bg-[#E69500] text-[#0A234F] text-xs sm:text-[13px] font-bold rounded-xl transition-all duration-250"
              aria-label="Search"
            >
        ...
        <Link
          to="/cart"
          className="lg:hidden relative p-2.5 text-white/75 hover:text-[#F5A300] hover:-translate-y-0.5 hover:bg-white/[0.07] rounded-xl transition-all shrink-0"
          aria-label={`Shopping cart${cartCount > 0 ? `, ${cartCount} items` : ''}`}
        >
          <ShoppingCart className="h-5 w-5" aria-hidden="true" />
          {cartCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-[#F5A300] text-[#0A234F] text-[10px] font-bold flex items-center justify-center px-0.5" aria-hidden="true">
              {cartCount}
            </span>
          )}
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-1.5 shrink-0">
          <Button variant="ghost" size="sm" className={ghostButtonClass} asChild>
          ...
          <Link
            to="/cart"
            className="relative p-2.5 text-white/75 hover:text-[#F5A300] hover:-translate-y-0.5 hover:bg-white/[0.07] rounded-xl transition-all"
            aria-label={`Shopping cart${cartCount > 0 ? `, ${cartCount} items` : ''}`}
          >
            <ShoppingCart className="h-5 w-5" aria-hidden="true" />
            {cartCount > 0 && (
          ...
          {user ? (
            <>
              ...
              <Button variant="ghost" size="sm" className={ghostButtonClass} onClick={handleLogout}>
            ...
            <>
              ...
              <Button
                size="sm"
                className="h-9 bg-[#F5A300] hover:bg-[#E69500] border border-[#F5A300]/40 text-[#0A234F] font-bold px-5 rounded-xl shadow-[0_6px_16px_rgba(245,163,0,0.22)] hover:-translate-y-0.5 transition-all duration-250 ml-1"
                asChild
              >
      ...
      <nav aria-label="Category navigation" className="hidden md:block border-t border-white/10">
                ...
                return (
                  ...
                  >
                    <Link
                      to={link.to}
                      className={[
                        "nav-cat-link text-[13px] font-semibold text-white/82 hover:text-[#F5A300] hover:-translate-y-px hover:bg-white/[0.07] px-3 py-2 rounded-lg transition-all duration-200 whitespace-nowrap text-center w-full",
                        isHovered ? "bg-white/[0.07] text-[#F5A300]" : "",
                      ].join(" ")}
                    >
                      {link.label}
                    </Link>
                    {hasChildren && isHovered && (
                      <div
                        className="absolute top-full left-0 z-50 min-w-[180px] rounded-xl border border-white/12 bg-[#0A234F] shadow-2xl overflow-hidden"
                        style={{ marginTop: "2px" }}
                        ...
                        {catNode.children.map((child) => (
                          ...
                          >
                            ...
                            {child.children?.length > 0 && (
                              <ChevronRight className="h-3.5 w-3.5 text-[#F5A300] shrink-0" aria-hidden="true" />
                            )}
                          </Link>
                        ))}
                      </div>
