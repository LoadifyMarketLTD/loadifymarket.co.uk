/**
 * TrendingProducts — top-viewed active listings.
 * Sorted by views descending, 2-column mobile grid.
 */
...
function ProductCard({ item, badge }: { item: HomeProduct; badge?: "Hot" | "New" | null }) {
        ...
        {badge && (
          <span
            className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full ${
              badge === "Hot"
                ? "bg-[#FBBF24] text-[#0B0F1A]"
                : "bg-white/90 text-[#0B0F1A]"
            }`}
          >
            {badge === "Hot" ? <Flame className="inline h-2.5 w-2.5 mr-0.5 -mt-px" aria-hidden="true" /> : null}
            {badge}
          </span>
        )}
      </div>
...
export default function TrendingProducts() {
  ...
  useEffect(() => {
    supabase
      .from("products")
      .select("id, title, price, images, slug, location, views, seller:users!sellerId(isVerified)")
      .eq("isActive", true)
      .eq("isApproved", true)
  ...
  return (
    <section aria-labelledby="trending-heading" className="px-4 py-5" style={{ background: "#0B0F1A" }}>
      <div className="flex items-center justify-between mb-3">
        <h2 id="trending-heading" className="text-[13px] font-black text-white uppercase tracking-widest flex items-center gap-1.5">
          <Flame className="h-4 w-4 text-[#FBBF24]" aria-hidden="true" />
          Trending
        </h2>
        <Link to="/catalog" className="text-[11px] font-bold text-[#FBBF24] uppercase tracking-wide hover:underline">
          See All →
        </Link>
      </div>
      {loading ? (
        ...
        <p className="text-slate-400 text-xs text-center py-6">No trending products yet.</p>
      ) : (
        <div className="grid grid-cols-2 gap-3">
          {products.map((item) => (
            <ProductCard key={item.id} item={item} badge="Hot" />
          ))}
        </div>
      )}
    </section>
