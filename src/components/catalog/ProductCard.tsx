...
const ProductCard = ({ product, linkState }: { product: Product; linkState?: Record<string, unknown> }) => {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      className="group bg-card rounded-xl border border-border hover:border-primary/40 hover:shadow-[0_0_22px_rgba(251,191,36,0.14),0_12px_28px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer"
      ...
      <div className="p-4 space-y-3">
        ...
        {isOwner ? (
          ...
          <Link to={`/product/${product.id}`} state={linkState ?? undefined}>
            <Button className="w-full bg-[linear-gradient(135deg,#D4AF37,#D4AF37)] text-[#0A0E1A] font-bold hover:shadow-[0_0_18px_rgba(251,191,36,0.28)] hover:opacity-90 transition-all duration-250 text-sm" size="sm">
              View Details
            </Button>
          </Link>
        )}
