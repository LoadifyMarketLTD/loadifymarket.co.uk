...
describe("ECN cross-border domain foundation", () => {
  ...
  it("keeps the existing UK domestic route live", () => {
    expect(migration).toContain("('GB-GB','GB','GB','live',true,true,true,true,true");
  });

  it("keeps every Romania route fail-closed", () => {
    expect(migration).toContain("('RO-RO','RO','RO','prelaunch',true,true,false,false,false");
    expect(migration).toContain("('GB-RO','GB','RO','prelaunch',false,false,false,false,false");
    expect(migration).toContain("('RO-GB','RO','GB','prelaunch',false,false,false,false,false");
  ...
  it("exposes only a service-role baseline decision interface", () => {
    ...
    expect(migration).toContain("GRANT EXECUTE ON FUNCTION public.server_market_route_baseline_v1(text,text)");
